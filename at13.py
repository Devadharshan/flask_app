...
cpu_usage_metric = meter.create_observable_gauge(
    "process_cpu_usage",
    callbacks=[lambda: [(psutil.cpu_percent(), {"metric_type": "cpu_usage"})]],
...
memory_usage_metric = meter.create_observable_gauge(
    ...
    callbacks=[
        lambda: [
            (psutil.virtual_memory().used / (1024 * 1024), {"metric_type": "memory_used"}),
            (psutil.virtual_memory().available / (1024 * 1024), {"metric_type": "memory_available"}),
        ]
    ],
